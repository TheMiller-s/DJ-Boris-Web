/**
 * EasyQRCodeJS / QRCode.js standalone lightweight bundle
 * MIT License
 */
var QRCode;
(function(){
...
MAX_LENGTH:[[{1:17,2:14,3:11,4:7},{1:32,2:26,3:20,4:14},{1:53,2:42,3:32,4:24},{1:78,2:62,3:46,4:34},{1:106,2:84,3:60,4:44},{1:134,2:106,3:74,4:58},{1:154,2:122,3:86,4:64},{1:192,2:152,3:108,4:84},{1:230,2:180,3:130,4:98},{1:271,2:213,3:151,4:119}]],
...
getLostPoint:function(qrCode){var moduleCount=qrCode.getModuleCount();var lostPoint=0;for(var row=0;row<moduleCount;row++){for(var col=0;col<moduleCount;col++){var sameCount=0;var dark=qrCode.isDark(row,col);for(var r=-1;r<=1;r++){if(row+r<0||moduleCount<=row+r)continue;for(var c=-1;c<=1;c++){if(col+c<0||moduleCount<=col+c)continue;if(r===0&&c===0)continue;if(dark===qrCode.isDark(row+r,col+c))sameCount++;}}if(sameCount>5)lostPoint+=(3+sameCount-5);}}for(var r2=0;r2<moduleCount-1;r2++){for(var c2=0;c2<moduleCount-1;c2++){var count=0;if(qrCode.isDark(r2,c2))count++;if(qrCode.isDark(r2+1,c2))count++;if(qrCode.isDark(r2,c2+1))count++;if(qrCode.isDark(r2+1,c2+1))count++;if(count===0||count===4)lostPoint+=3;}}return lostPoint;}};
...
QRCodeModel.prototype={addData:function(data){this.dataList.push(new QR8bitByte(data));this.dataCache=null;},isDark:function(row,col){return this.modules[row][col];},getModuleCount:function(){return this.moduleCount;},make:function(){
if(this.typeNumber<1){var typeNumber=1;for(typeNumber=1;typeNumber<40;typeNumber++){for(var i=0;i<this.dataList.length;i++){}var rsBlocks=QRRSBlock.getRSBlocks(typeNumber,this.errorCorrectLevel);var buffer=new QRBitBuffer();var totalDataCount=0;for(var j=0;j<rsBlocks.length;j++)totalDataCount+=rsBlocks[j].dataCount;for(var k=0;k<this.dataList.length;k++){var data=this.dataList[k];buffer.put(data.mode,4);buffer.put(data.getLength(),QRUtil.getLengthInBits(data.mode,typeNumber));data.write(buffer);}if(buffer.getLengthInBits()<=totalDataCount*8)break;}this.typeNumber=typeNumber;}
...
setupTypeInfo:function(test,maskPattern){var data=(this.errorCorrectLevel<<3)|maskPattern;var bits=QRUtil.getBCHTypeInfo(data);for(var i=0;i<15;i++){var mod=(!test&&((bits>>i)&1)===1);if(i<6){this.modules[i][8]=mod;}else if(i<8){this.modules[i+1][8]=mod;}else{this.modules[this.moduleCount-15+i][8]=mod;}if(i<8){this.modules[8][this.moduleCount-i-1]=mod;}else if(i<9){this.modules[8][15-i-1+1]=mod;}else{this.modules[8][15-i-1]=mod;}}this.modules[this.moduleCount-8][8]=(!test);},
